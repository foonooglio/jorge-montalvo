...
export default function WorkersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [farmsRes, empsRes] = await Promise.all([
      supabase.from('prg_farms').select('id, name').order('name'),
      supabase.from('prg_employees').select('*').order('name'),
    ])
  ...
  const addEmployee = async (e: React.FormEvent) => {
    ...
    const { data } = await supabase.from('prg_employees').insert({
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#15803d', margin: '0 0 16px' }}>👥 Workers</h2>
        ...
        <div key={farm.id} style={{ marginBottom: 20 }}>
          ...
          <div style={{
            ...
            {empsByFarm(farm.id).length === 0 ? (
              <p style={{ color: '#9ca3af', padding: '10px 12px', fontSize: 13, margin: 0 }}>No workers assigned.</p>
            ) : (
      ...
      <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 16 }}>
        <h3 style={{ fontSize: 15, fontWeight: 600, margin: '0 0 12px', color: '#374151' }}>+ Add Worker</h3>
